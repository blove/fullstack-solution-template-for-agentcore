...
class CopilotKitMiddleware(AgentMiddleware[StateSchema, Any]):
    ...
    def after_agent(
        ...
        return {
            "messages": updated_messages,
            "copilotkit": {
                # Keep intercepted_tool_calls populated so the checkpoint
                # saver can identify frontend tool calls and avoid injecting
                # spurious "interrupted before completion" error ToolMessages.
                "intercepted_tool_calls": intercepted_tool_calls,
                "original_ai_message_id": original_message_id,
            },
        }
